...
@Component({
  ...
  template: `
    <app-page-shell [title]="'nav.positions' | translate" [breadcrumbs]="breadcrumbs()">
      <div class="actions-row" actions>
        @if (canCreate()) {
          <button type="button" class="ds-btn ds-btn--primary ds-btn--sm" (click)="openCreate()">{{ 'common.add' | translate }} {{ 'table.position' | translate }}</button>
        }
      </div>
      <div filters>
                ...
                <tr>
                  ...
                  <td class="cell-vacant">
                    @if (canEdit()) {
                      <button
                        type="button"
                        class="ds-btn ds-btn--ghost ds-btn--icon vacant-btn"
                        ...
                        [title]="(p.isVacant ? 'positions.setOccupied' : 'positions.setVacant') | translate"
                      >
    ...
    @if (showModal()) {
      <div class="modal-overlay" (click)="closeModal()">
        <div class="modal-drawer ds-card" (click)="$event.stopPropagation()">
          <h3 class="modal-drawer__title">{{ editingId() ? ('common.edit' | translate) : ('common.add' | translate) }} {{ 'table.position' | translate }}</h3>
          @if (modalError()) {
